...
- (UIView *)alertView {
    if (!_alertView) {
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_BOUNDS.size.width - kTopViewHeight - kDatePicHeight, SCREEN_BOUNDS.size.width, kTopViewHeight + kDatePicHeight)];
...
- (UIView *)topView {
    if (!_topView) {
        _topView =[[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_BOUNDS.size.width, kTopViewHeight + 0.5)];
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        _rightBtn.frame = CGRectMake(SCREEN_BOUNDS.size.width - 65, 8, 60, 28);
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(65, 0, SCREEN_BOUNDS.size.width - 130, kTopViewHeight)];
...
- (UIView *)lineView {
    if (!_lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(0, kTopViewHeight, SCREEN_BOUNDS.size.width, 0.5)];
